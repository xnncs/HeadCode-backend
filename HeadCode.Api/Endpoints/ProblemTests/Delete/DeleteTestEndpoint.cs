...
public class DeleteTestEndpoint : Endpoint<DeleteTestRequest, Results<Ok, NotFound, InternalServerError>>
{
    ...
    public override async Task<Results<Ok, NotFound, InternalServerError>> ExecuteAsync(DeleteTestRequest request, CancellationToken cancellationToken)
    {
        await using IDbContextTransaction transaction = await _dbContext.Database.BeginTransactionAsync(cancellationToken);
        try
        {
            int count = await _dbContext.ProblemTests.Where(x => x.Id == request.Id).ExecuteDeleteAsync(cancellationToken);
