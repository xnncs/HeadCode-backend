...
{
    public RegisterEndpoint(ApplicationDbContext dbContext, ILogger<RegisterEndpoint> logger, IPasswordHasher passwordHasher)
    {
    ...
    public override async Task<Results<Ok, BadRequest<string>, InternalServerError>> HandleAsync(RegisterRequest request, CancellationToken cancellationToken)
    {
        ...
        if (emailAlreadyUsed)
        {
            return TypedResults.BadRequest<string>("Email already used");
        }
